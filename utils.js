...
function formatMessage(m) {
  ...
  var obj = {
    ...
    participantIDs: (originalMessage.group_thread_info ? originalMessage.group_thread_info.participant_ids.map(function(v) {return v.toString();***REMOVED*** : [originalMessage.sender_fbid]),
    ...
    return bluebird.try(function() {
      ...
      if (data.statusCode >= 500 && data.statusCode < 600) {
        ...
        if (data.request.headers['Content-Type'].split(";")[0] === "multipart/form-data") {
          return defaultFuncs
            .postFormData(url, jar, data.request.formData, {***REMOVED***
...
function getName(data) {
  if (data.name) {
    ...
    return data.participants.map(function(v) { return v.replace('fbid:', ''); ***REMOVED***.join(', ');
  }
  return '';
}
...
function formatThread(data) {
  return {
    ...
    participantIDs: data.participants.map(function(v) { return v.replace('fbid:', ''); ***REMOVED***,
